...
export async function enrichAllPropsCollection({ season, week, skipEnriched }: any) {
  ...
  try {
    ...
    const props = snapshot.docs.map(d => ({ id: d.id, ...d.data() })) as (NFLProp & {id: string})[];
