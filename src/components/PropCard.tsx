...
import { PropDoc, BetLeg } from '@/lib/types';
...
interface PropCardProps {
  ...
  onEdit?: (prop: PropDoc) => void;
  onDelete?: (id: string) => void;
  isAdded?: boolean;
  isAdmin?: boolean;
}

const formatOdds = (o?: number) =>
  o == null ? '-110' : o > 0 ? `+${o}` : `${o}`;
...
export const PropCard = ({
  ...
  const handleAdd = (selection: 'Over' | 'Under', odds: number) => {
    const leg: BetLeg = {
      ...
      player: prop.player,
      prop: prop.prop,        // <-- FIXED (no propId)
      line: prop.line,
      odds,
      status: 'pending',
      gameDate: prop.gameDate ?? undefined,
      league: prop.league
    };
    ...
    toast.success('Added to bet slip', {
      description: `${prop.player} ${selection} ${odds}`
    });
  };

  return (
    <article
      className="bg-slate-900 border border-white/10 rounded-2xl p-4 hover:border-indigo-500/50 transition-all"
      role="group"
      aria-labelledby={`prop-${prop.id}`}
    >
      <div className="flex justify-between items-start mb-3">
        <div>
          <h4 id={`prop-${prop.id}`} className="font-bold text-sm text-white">
            {prop.player}
          </h4>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className={`flex flex-col items-center p-2 rounded-xl border transition-all group ${
            isAdded
              ...
              : 'bg-white/5 hover:bg-emerald-500/10 hover:border-emerald-500/50'
          }`}
        >
          <span className="text-[9px] uppercase font-bold text-slate-500 group-hover:text-emerald-400">
            Over
          </span>
        ...
        <button
          ...
          className={`flex flex-col items-center p-2 rounded-xl border transition-all group ${
            isAdded
              ? 'opacity-60 cursor-not-allowed'
              : 'bg-white/5 hover:bg-red-500/10 hover:border-red-500/50'
          }`}
        >
          <span className="text-[9px] uppercase font-bold text-slate-500 group-hover:text-red-400">
            Under
          </span>
          <span className="text-xs font-mono font-bold text-white">
