...
export function BetSlipPanel({ week }: BetSlipPanelProps) {
  ...
  const decimalOdds = isEmpty ? 1 : toDecimal(totalParlayOdds.toString());
            ...
            <div className="divide-y divide-border">
                ...
                <div key={(leg as any).id || (leg as any).propId}
                    ...
                    {(leg as any).odds != null && (
                        ...
                        (leg as any).odds > 0 ? 'text-profit' : 'text-muted-foreground'
                        }`}>
        ...
        {!isEmpty && (
            ...
            <div className="space-y-1">
                <div className="flex justify-between items-center">
                    ...
                    {kelly.recommendedStake > 0 && (
                        <span className="text-[10px] font-mono text-primary/70">1/4 Kelly Stake Applied</span>
                    )}
                </div>
                <div className="flex items-center gap-2 bg-secondary rounded-lg px-3 py-2 border border-border">
                <span className="text-sm text-muted-foreground">$</span>
                <input type="number" inputMode="decimal" value={stake}
            ...
            <div className="space-y-2 pt-3 border-t border-border">
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                    ...
                    <span className="font-bold tabular-nums">{totalParlayOdds > 0 ? '+' : ''}{totalParlayOdds.toFixed(0)}</span>
                </div>
                {selectedBonus && (
            ...
            <button 
                ...
                className="w-full bg-[#FFD700] hover:bg-[#e6c200] active:scale-[0.98] 
                           ...
                           flex items-center justify-center gap-2 italic tracking-tighter"
            >
                {loading ? (
