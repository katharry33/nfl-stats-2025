...
import { Trash2, X, ChevronRight, Zap } from 'lucide-react';
...
export function BetSlipPanel() {
  ...
  // Calculate total odds (assuming $10 base wager for preview)
  const estPayout = useMemo(() => {
    if (selections.length === 0) return 0;
    // Simple parlay math (very basic for preview)
    const multiplier = selections.reduce((acc, leg) => {
      const odds = leg.odds;
  ...
  if (!mounted || !selections || selections.length === 0) return null;

  return (
    <div className="fixed bottom-6 right-6 w-[380px] bg-[#0d0d0d] border border-white/10 rounded-[32px] shadow-2xl z-50 flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 duration-500">
      ...
      <div className="p-5 flex items-center justify-between bg-white/3 border-b border-white/5">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => clearSlip()} className="p-2 hover:bg-red-500/10 text-slate-500 hover:text-red-400 transition-colors rounded-lg">
            <Trash2 size={14} />
          </button>
          <button 
      ...
      {isExpanded && (
        <div className="max-h-[400px] overflow-y-auto p-4 space-y-3 custom-scrollbar">
          {selections.map((bet) => (
            <div key={bet.id} className="group relative p-4 rounded-2xl bg-white/2 border border-white/5 hover:border-white/10 transition-all">
              <button onClick={() => removeLeg(bet.id)} className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 p-1 hover:bg-red-500/20 text-red-400 rounded-md transition-all">
                <X size={12} />
              </button>
              <div className="flex flex-col gap-1">
                <span className="text-[9px] font-black uppercase tracking-widest text-[#FFD700]">
                  {bet.player} • {bet.league || 'NFL'}
                </span>
                ...
                <p className="text-[9px] font-bold text-slate-600 uppercase tracking-tighter mt-1">{bet.matchup}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="p-5 bg-black/40 border-t border-white/5 space-y-4">
        <div className="flex items-center justify-between px-1">
          <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Est. Payout ($10)</span>
          <span className="text-sm font-black text-white italic">${estPayout}</span>
        </div>
