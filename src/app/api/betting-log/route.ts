...
// FIX 4: handle "Oct 02 2025", "Dec 29 2025" date strings in addition to ISO
function parseDate(raw: any): string | null {
  ...
  if (typeof d === 'string') {
    ...
    // Human-readable: "Oct 02 2025", "Dec 29 2025", "October 2, 2025" etc.
...
// FIX 2: prefer result field when status is "pending" but result has real value
function parseStatus(raw: any): string {
  ...
  // If status is pending/empty but result has a real answer, use result
...
function normalizeLegItem(l: any, fallbackRaw: any, docId: string, i: number): any {
  ...
  // FIX 3: if embedded leg has empty selection, derive from line string
...
function buildBetFromDoc(doc: any, raw: any): any {
  // FIX 1: use || not ?? so stake:0 falls through to wager
  ...
  return {
    ...
    week:          Number(raw.week) || null,
    status:        parseStatus(raw),
    cashOutAmount: raw.cashOutAmount || null,
    odds:          odds || null,
    stake,
    boost:         raw.boost,
...
function buildParlayFromEntries(parlayId: string, entries: { doc: any; raw: any }[]): any {
  // FIX 1: use || not ?? so stake:0 falls through to wager when finding header
  ...
  const legs = entries.map(({ doc, raw }) => {
    ...
    // Derive selection from embedded legs array if top-level is empty
    let legSelection = raw.selection || selection;
    if (!legSelection && Array.isArray(raw.legs) && raw.legs.length > 0) {
      const embedded = raw.legs[0];
      legSelection = embedded.selection ||
        (typeof embedded.line === 'string' && embedded.line.toLowerCase().includes('over') ? 'Over' :
         typeof embedded.line === 'string' && embedded.line.toLowerCase().includes('under') ? 'Under' : '');
    }
    // Last resort: derive from top-level line string
  ...
  // FIX 2: use parseStatus on header which now correctly prefers result over pending status
  ...
  return {
    ...
    status:        parlayStatus,
    cashOutAmount: hr.cashOutAmount || null,
    odds:          odds || null,
    stake,
    boost:         hr.boost,
...
export async function GET(request: Request) {
  try {
    ...
    // Sort by date desc — fall back to gameDate then date string if createdAt missing
    allBets.sort((a, b) => {
